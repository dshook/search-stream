    ...
    if(typeof obj === 'object'){
      if(Array.isArray(obj)){
        ...
        for(var i = 0; i < obj.length; i++){
          if(search(options, filter, obj[i], null)) res.push(obj[i]);
        }
        return res;
      }else{
        return search(options, filter, obj, null);
      }
      ...
      searchTransform._transform = function (chunk, encoding, done) {
        if(search(options, filter, chunk, null)) this.push(chunk);
        done();
      };
...
var search = function(options, filter, obj, objKey){
  if(isTestableType(obj)){
    //if search keys are specified and there's an objKey from higher level object search
    //make sure it's a key that's good
    if(
      options.searchKeys.length > 0
      && objKey != null
      && !arrayContains(objKey, options.searchKeys)
    ){
      return false;
    }
    ...
    if(Array.isArray(obj)){
      ...
      //return true for any array value that matches passing down the object key
      for(var i = 0; i < obj.length; i++){
        if(search(options, filter, obj[i], objKey)){
          return true;
        }
      }
    }else{
      ...
      for(var key in obj){
        ...
        if(isTestableType(value)){
          ...
          search(options, filter, value, key)
        ){
          return true;
        }
      }
